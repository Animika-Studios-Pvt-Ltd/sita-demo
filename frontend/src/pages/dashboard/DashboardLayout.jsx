    ...
    <div
      ...
      {isMobile && mobileSidebar && (
        <div
          ...
          onClick={() => setMobileSidebar(false)}></div>
      )}
      <aside
        className={`fixed top-0 h-screen bg-white/70 backdrop-blur-xl text-slate-700 flex flex-col justify-between pt-4 pb-6 px-3 shadow-[0_20px_45px_-30px_rgba(15,23,42,0.45)] z-40 transition-all duration-300 ${
          isMobile
            ? mobileSidebar
              ? "w-64 left-0"
              : "w-0 -left-64"
        ...
        <div>
          <div
            ...
            <div className="flex flex-col items-center gap-2">
              <img
                ...
                alt="Admin Logo"
                className={`w-full ${
                  isExpanded || mobileSidebar ? "max-w-[110px]" : "max-w-[52px]"
          ...
          <nav className="space-y-2">
            <Link
              ...
              className={`no-underline flex items-center ${
                isExpanded || mobileSidebar
                  ? "gap-4 px-4"
                  : "justify-center px-2"
              } py-2.5 rounded-xl transition-all duration-200 border border-transparent backdrop-blur-md ${
                isActive("/dashboard")
              ...
              {isActive("/dashboard") && (
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#7A1F2B]/60"></div>
              )}
            ...
            <Link
              ...
              {(isExpanded || mobileSidebar) && (
                <span className="text-sm font-medium">CMS Module</span>
              )}
            </Link>
          </nav>
        </div>
        ...
        <div
          className={`${isExpanded || mobileSidebar ? "px-4 space-y-2" : "flex flex-col items-center space-y-2"} transition-all duration-300 border-t border-white/60 pt-6`}>
          <Link
            to="/dashboard/settings/mfa"
            className={`no-underline flex items-center gap-3 px-4 py-2.5 rounded-xl w-full transition-all duration-300 border border-transparent backdrop-blur-md ${
              isActive("/dashboard/settings/mfa")
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-2.5 text-[#8b171b] hover:text-[#8b171b] hover:bg-red-50/70 rounded-xl w-full transition-all duration-300 border border-transparent backdrop-blur-md hover:border-red-200/60 hover:ring-1 hover:ring-red-200/60 ${
              isExpanded || mobileSidebar ? "justify-start" : "justify-center"
            }`}
            aria-label="Logout">
      ...
      <div
        className={`flex-1 flex flex-col transition-all duration-300 ${
          isExpanded && !isMobile ? "ml-64" : !isMobile ? "ml-20" : "ml-0"
        }`}>
        <header
          className={`fixed top-0 z-20 flex flex-col items-center justify-center px-6 py-4 bg-white/70 backdrop-blur-xl border-b border-white/70 shadow-sm transition-all duration-300 ${
            isExpanded && !isMobile
              ? "left-64"
              : !isMobile
                ? "left-20"
          ...
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
            <button
              ...
              className={`relative inline-flex items-center w-16 h-8 rounded-full border-1 shadow-sm overflow-hidden transition-all duration-200 ${
                adminTheme === "dark"
              ...
              <span
                className={`absolute top-0.5 left-0.5 h-7 w-7 rounded-full flex items-center justify-center shadow-md transition-transform duration-200 ${
                  adminTheme === "dark"
