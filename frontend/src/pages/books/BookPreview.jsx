...
const BookPreview = () => {
  ...
  const buildPagesFromChapters = (chapters, maxHeight) => {
    if (!chapters || chapters.length === 0) {
      return splitTextByHeight(book?.description || "", maxHeight).map(
        (text) => ({
          type: "content",
          text,
        }),
  ...
  const flipbookPagesMobile = [
    { type: "front" },
    ...mobilePages,
    { type: "back" },
  ];
  const flipbookPagesTablet = [
    { type: "front" },
    ...tabletPages,
    { type: "back" },
  ];
  ...
  if (desktopPages.length % 2 !== 0)
    flipbookPagesDesktop.push({ type: "blank" });
  ...
  return (
    <div className="container mx-auto px-4 mb-20" data-aos="fade-up">
      ...
      <div
        className={`relative flex justify-center items-start px-4 min-h-[70vh] ${
          screenSize >= 1440 ? "mt-4" : "mt-4"
        }`}>
