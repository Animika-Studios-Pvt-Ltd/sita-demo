            ...
            <div className="p-6 md:p-8 flex flex-col flex-grow">
                ...
                <div className="pt-6 border-t border-slate-100 mt-auto flex justify-between items-center group/btn">
                    {isUpcoming ? (
                        ...
                        >
                            {embedUrl ? "Play Episode" : "Listen on Platform"} <ArrowRight size={16} />
                        </button>
                    )}
...
const PodcastsPage = () => {
    ...
    const fetchPodcasts = async () => {
        try {
            ...
            const sorted = activePodcasts.sort((a, b) => new Date(b.releaseDate) - new Date(a.releaseDate));
                    ...
                    {podcasts.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
                            {currentPodcasts.map((podcast, index) => (
                                <PodcastCard key={podcast._id} podcast={podcast} index={index} />
                            ))}
                        </div>
                    ) : (
